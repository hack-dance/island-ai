...
export const customComponents = {
  ...
  h2: ({ className, ...props }: React.HTMLAttributes<HTMLHeadingElement>) => (
    <h2
      className={cn(
        "font-heading mt-8 scroll-m-20 text-2xl font-semibold tracking-tight first:mt-0",
        className
      )}
      {...props}
    />
  ...
  pre: ({ className, ...props }: React.HTMLAttributes<HTMLElement>) => (
    <pre
      className={cn(
        "mb-8 mt-2 max-h-[650px] overflow-auto rounded-lg border py-2 px-1 bg-zinc-800",
        className
      )}
      {...props}
    />
