...
SCHEMA_STREAM_VERSION="4.1.0"

mkdir -p "$TEMP_DIR/packages/zod-stream" "$TEMP_DIR/packages/stream-hooks" "$TEMP_DIR/packages/llm-polyglot" "$TEMP_DIR/packages/evalz" "$TEMP_DIR/tarballs" "$TEMP_DIR/consumer" "$TEMP_DIR/schema-consumer" "$TEMP_DIR/sdk-consumer"

for package in zod-stream stream-hooks; do
  source_dir="$ROOT/public-packages/$package"
  if [[ "$package" == "stream-hooks" ]]; then
...
node -e 'const fs=require("node:fs"); for (const name of ["zod-stream","stream-hooks"]) { const path=process.argv[1]+"/packages/"+name+"/package.json"; const pkg=JSON.parse(fs.readFileSync(path,"utf8")); pkg.version="4.0.0"; if (name === "stream-hooks") pkg.peerDependencies["zod-stream"]="^4.0.0"; delete pkg.devDependencies; fs.writeFileSync(path,JSON.stringify(pkg,null,2)+"\n"); }' "$TEMP_DIR"
...
npm install --ignore-scripts --no-audit --no-fund \
  "schema-stream@$SCHEMA_STREAM_VERSION" \
...
npm install --ignore-scripts --no-audit --no-fund "schema-stream@$SCHEMA_STREAM_VERSION" zod@3.25.76 typescript@5.9.3 >/dev/null
